...
require 'breadcrumber/configure'
...
module Breadcrumber
  ...
  def add_literal_breadcrumb name, path = ''
    ...
    name = name.truncate(Breadcrumber.truncate_length, separator: Breadcrumber.truncate_separator) unless name =~ /\<(.*)\>/
...
ActionController::Base.send :include, Breadcrumber
